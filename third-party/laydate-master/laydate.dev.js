...
var frameDoc = document; // added by chentao 01/26/2015
//主接口
win.laydate = function(options){
    options = options || {};
    if(!options.elemIsId){ //added by chentao 01/26/2015
        options.elemIsId = 'true';
        frameDoc = document;
    }else{
        frameDoc = options.elem.ownerDocument;
    }
    try{
...
Dates.run = function(options){
    ...
    elem = options.elemIsId == 'true' ? (options.elem ? S(options.elem) : target) : (options.elem ? options.elem : target); //modified by chentao 01/26/2015
...
Dates.orien = function(obj, pos){
    ...
    var rect = getClientRect(Dates.elem); //added by chentao 01/26/2015
...
Dates.events = function(){
    ...
    Dates.on(frameDoc, 'mouseup', function(){
        var box = S('#'+as[0]);
        if(box && box.style.display !== 'none'){
            Dates.check() || Dates.close();
        }
    }).on(frameDoc, 'keydown', function(event){
        //event = event || win.event;
        //var codes = event.keyCode;

        //如果在日期显示的时候按回车
        //if(codes === 13 && Dates.elem){
            //Dates.creation([Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]]);   hidden by chentao 01/26/2015
        //}
    });
};
